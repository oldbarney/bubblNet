...
surveydb=createBubNetTables.getSurveydb()
surveytimestamped=False
config={}
...
def loadBSMs():
    ...
    for spec in specs:
        ...
        md=int(spec[1])
        if md&1==1:
            bsms.append(BSM(n,"/home/pi/bsmdb.db",db,dm))
        if md&2==2:
            testbsms.append(BSM(n,"/home/pi/bsmdb.db",db,dm))
...
class Remote_Sensor:
    def updatedb(self,d1,d2):
        ...
        t=time.gmtime()
        db.execute("UPDATE readings SET svalue=(?),value=(?),batt=(?),sbatt=(?),time=(?) WHERE channel = (?);",(sval,val,batt,sbatt,time.asctime(t),self.idNo))
        db.commit()
        log(1,"Sensor data logged from %s"%(self.idNo))
        hh=secSince(t)//3600
        day=hh//24
        curs=surveydb.cursor()
        curs.execute("SELECT h0 FROM survey WHERE day=(?) AND channel=(?);",(day,self.idNo))
        if curs.fetchall()==[] :
            curs.execute("INSERT INTO survey (channel,day) VALUES((?),(?));",(self.idNo,day))
        if hh==self.lastHour:
            curs.close()
            return
        curs.execute("UPDATE survey SET h{} = (?) WHERE day = (?) AND channel = (?);".format(hh%24),(val,day,self.idNo))
        curs.close()
        surveydb.commit()
        self.lastHour=hh
...
def handle(ser,idNo,code,d1,d2,pipe):
    ...
    if pipe!=0 :
        return
    if idNo==0 and code==1 :  #Signal strength Measurement
        tx(serialNo,0,1,d1,d2,1)
        return
    if ser!=serialNo :
    ...
    if idNo in sensors:
        ...
        if idNo==0 or idNo==210 :
            dm.enqueue(("Error:idNo="+str(idNo),64))
            log(2,"Error - invalid idNo:"+str(idNo))
            return
        if code>=128:
...
def exerciseControl(endRxFunc):  
#call endRxFunc and return True if control from database acted on,
...
def bsmHandler(endRxFunc):
    ...
    #        log(0,"running test bsm events")
            bsm.runEvent()
    return exerciseControl(endRxFunc)   # mode=="manual":
...
#Perform background activities to maintain bubblNet
#Call endRxFunc and return true if message sent, otherwise pass this behaviour to bsmHandler
def idle(endRxFunc):
    tdisp=dm.dequeue()
    if tdisp != None:
        mess,pos=tdisp
