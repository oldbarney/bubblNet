...
#executebsm version 1.1
#Changes in v 1.1  -Events are persistant so that values can be passed. This means they need to be manually cleared by the responder
import time
import calendar
import sqlite3
import smtplib
...
#            self.log("checking for event "+varname[5:]+" in "+repr(BSM.ipc_events))
            key=varname[5:]
            if key in BSM.ipc_events:
                return BSM.ipc_events[key]
            else:
                return None
        if varname == "button":
