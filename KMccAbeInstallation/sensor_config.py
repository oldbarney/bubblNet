...
def getTimeErr(channel) :  #Channel is integral second-number within period
    ...
    phase=time.time()-period*(int(time.time()/period)) 
    phase=int(64*phase)   #Units of 1/64th Sec
    err=((phase-64*channel+period*32)%(period*64))-period*32
    print("err={}".format(err))
    return err

sensors=[None for _ in range(period)]
...
def sub_handler(ser,idNo,code,d1,d2,pipe):
    print("Pipe "+str(pipe))
...
def handler(ser,idNo,code,d1,d2,pipe):
    if ((pipe!=0) or ((ser==serialNo) and (idNo!=0xFF))):
        return
    time.sleep(0.002)
    tx(ser,idNo,255,0xFFFF,0xFFFF,1)
    print("hPipe "+str(pipe))
    print("hSerial number "+str(ser))
    print("hidNo="+str(idNo))
    print("hcode="+str(code))
    print("hd1=%s"%d1)
    print("hd2=%s"%d2)
    rx(sub_handler)
    try :
        newId=int(input("idNo ("+str(idNo)+") >"))
    except:
        print("Not reconfiguring")
        flushRx()
        tx(ser,idNo,0,ser,idNo|(code<<8),1)
        return
    print( "Re-configuring")
    flushRx()
    tx(ser,idNo,254,serialNo,newId|(code<<8),1)
    rx(sub_handler)
