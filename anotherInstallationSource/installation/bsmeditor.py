...
def getMachineButs():
    ...
    cs=db.cursor()
    cs.execute("SELECT name FROM bsms;")
    ms=cs.fetchall()
    cs.close()
    res='<form action="bsmeditor.py" method="POST">'+'<br />'.join(['<input type="submit" value='+name[0]+' name="edit'+name[0]+'"><br />' for name in ms])+'</form>'
    return res
...
def getButtons():
    ...
    res+="&nbsp;"*4
    res+="<button onclick='reload();'>Exit (no save)</button>"
...
def save(name,text):
    ...
    db.execute("INSERT into bsms VALUES((?),(?),(?));",(name,text,1))
    db.commit()
    db.close()
...
def main():
    ...
    print('td { background-color: lightgreen; vertical-align: text-top; }')
    print('table {  empty-cells:show; }')
    print('button {  background-color: pink; }')
    print('</style>')
    print(getJS())
